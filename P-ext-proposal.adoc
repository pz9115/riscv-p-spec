...
The P extension does not define any new load or store instructions. Instead, it
relies on the existing GPR load/store instructions defined in the base ISA.

Misaligned accesses will be common in software using P extension instructions.
It is therefore recommended (but not required) that implementations support
misaligned accesses more efficiently than alternative code sequences that avoid
the misalignment.
...
|CLS |{nbsp} |count leading redundant sign bits
...
PABDSUMAU.B = P {ABD + SUM + Accum} U .B
...
* `>>` — logical right shift of a bit-vector, filling vacated bits with zeros.
* `shift_bits_right_arith(bv, shamt)` — arithmetic right shift of a
  bit-vector, filling vacated bits with copies of its most-significant bit.
  The result has the same width as `bv`. The shift amount is non-negative;
  an amount greater than or equal to the width produces all sign bits.
...
Arithmetic right shifts of bit-vectors use `shift_bits_right_arith`.
Sign-extending a bit-vector, logically shifting it right, and extracting
the low result bits is equivalent only when enough sign bits were added
for the shift amount and the required result width. Sign extension alone
does not change `>>` into an arithmetic shift.

For integer operands in the pseudocode, `x << n` denotes multiplication
by 2^n^, and `x >> n` denotes division by 2^n^ rounded toward negative
infinity, for non-negative `n`. These operations produce integer results
with no fixed bit-width. The function `to_bits(size, value)` converts an
integer to a bit-vector of the specified width.
...
to 32 bits and replicates it into every word element of `rd`.
...
with an 8-bit immediate and packed byte elements. This instruction uses a
register-pair destination `rd_p`.
...
uses a register-pair destination `rd_p`.
...
[source,pseudocode]
----
imm_h = zero_extend(16, imm[9:0]) << 6
...
loaded into the upper bits of each word element.
...
[source,pseudocode]
----
imm_w = zero_extend(32, imm[9:0]) << 22
...
This instruction uses a register-pair destination `rd_p`.
...
PADD.W is encoded in the OP-32 major opcode.
...
PADD.WS is encoded in the OP-IMM-32 major opcode.
...
scalar format with packed byte elements.
...
scalar format with packed halfword elements.
...
scalar format with word elements.
...
PSUB.W is encoded in the OP-32 major opcode.
...
SADDU is encoded in the OP-32 major opcode.
...
register-pair format with packed halfword elements. This instruction usesu
...
register-pair format. This instruction uses register pairs `rd_p`, `rs1_p`, and
`rs2_p`.
...
PSSUB.W is encoded in the OP-32 major opcode.
...
result is clamped to the range [-(2^31^), 2^31^-1].
...
PSSUB.DB is encoded in the OP-IMM-32 major opcode using the double-wide
register-pair format with packed byte elements. This instruction uses register
...
packed halfword elements. This instruction uses register pairs `rd_p`, `rs1_p`,
and `rs2_p`.
...
PSSUB.DW is encoded in the OP-IMM-32 major opcode using the double-wide
register-pair format. This instruction uses register pairs `rd_p`, `rs1_p`, and
`rs2_p`.
...
PSSUBU.W is encoded in the OP-32 major opcode.
...
element result is clamped to the range [0, 2^32^-1].
...
SSUBU is encoded in the OP-32 major opcode.
...
packed byte elements. This instruction uses register pairs `rd_p`, `rs1_p`, and
`rs2_p`.
...
PSSUBU.DH is encoded in the OP-IMM-32 major opcode using the double-wide
register-pair format with packed halfword elements. This instruction uses
register pairs `rd_p`, `rs1_p`, and `rs2_p`.
...
PSSUBU.DW is encoded in the OP-IMM-32 major opcode using the double-wide
register-pair format. This instruction uses register pairs `rd_p`, `rs1_p`, and
`rs2_p`.
...
PAADD.W is encoded in the OP-32 major opcode.
...
AADD is encoded in the OP-32 major opcode.
...
to `rd`. The RV64 counterpart is PAADD.W.
...
PAADD.DB is the register-pair (double-wide) variant of PAADD.B. It is encoded in
the OP-IMM-32 major opcode using the double-wide register-pair format with
...
PAADD.DH is the register-pair (double-wide) variant of PAADD.H. It is encoded ino
the OP-IMM-32 major opcode using the double-wide register-pair format with
register-pair operands.
...
PAADD.DW is the register-pair (double-wide) variant of PAADD.W. It is encoded in
the OP-IMM-32 major opcode using the double-wide register-pair format with
register-pair operands.
...
PAADDU.W is encoded in the OP-32 major opcode.
...
32 bits to the corresponding word of `rd`.
...
AADDU is encoded in the OP-32 major opcode.
...
shifts the result right by 1, and writes the lower 32 bits to `rd`. The RV64
counterpart is PAADDU.W.
...
PAADDU.DB is the register-pair (double-wide) variant of PAADDU.B.
...
PAADDU.DH is the register-pair (double-wide) variant of PAADDU.H. It is encoded
in the OP-IMM-32 major opcode using the double-wide register-pair format with
register-pair operands.
...
PAADDU.DW is the register-pair (double-wide) variant of PAADDU.W. It is encoded
in the OP-IMM-32 major opcode using the double-wide register-pair format with
register-pair operands.
...
PASUB.W is encoded in the OP-32 major opcoded.
...
ASUB is encoded in the OP-32 major opcode.
...
32 bits to `rd`. The RV64 counterpart is PASUB.W.
...
PASUBU.W is encoded in the OP-32 major opcode.
...
ASUBU is encoded in the OP-32 major opcode.
...
The RV64 counterpart is PASUBU.W.
...
PSH1ADD.W is encoded in the OP-32 major opcode.
...
PSSH1SADD.W is encoded in the OP-32 major opcode.
...
SSH1SADD is encoded in the OP-32 major opcode.
...
PAS.WX is encoded in the OP-32 major opcode.
...
is useful for complex number arithmetic.
...
PSA.WX is encoded in the OP-32 major opcode.
...
PSAS.WX is encoded in the OP-32 major opcode.
...
PSSA.WX is encoded in the OP-32 major opcode.
...
saturation. This instruction is useful for complex number arithmetic.
...
The truncated results are written to the corresponding words of `rd`.
...
truncated results are written to the corresponding words of `rd`.
...
PABD.DB is the register-pair (double-wide) variant of PABD.B.
...
PABD.DH is the register-pair (double-wide) variant of PABD.H.
...
PABDU.DB is the register-pair (double-wide) variant of PABDU.B.
...
PABDU.DH is the register-pair (double-wide) variant of PABDU.H.
...
PSABS.DB is the register-pair (double-wide) variant of PSABS.B. It is encoded as
a unary instruction in the OP-IMM-32 major opcode using the double-wide
register-pair format.
...
PSABS.DH is the register-pair (double-wide) variant of PSABS.H. It is encoded as
a unary instruction in the OP-IMM-32 major opcode using the double-wide
register-pair format.
...
PREDSUM.WS is encoded in the OP-IMM-32 major opcode.
...
PREDSUM.DBS reads a 64-bit packed source from the even-odd register pair
specified by `rs1_p` and produces a 32-bit scalar result in `rd`. If
`rs1_p = 0`, the source pair is treated as zero.
...
PREDSUM.DHS reads a 64-bit packed source from the even-odd register pair
specified by `rs1_p` and produces a 32-bit scalar result in `rd`. If
`rs1_p = 0`, the source pair is treated as zero.
...
PREDSUMU.WS is encoded in the OP-IMM-32 major opcode.
...
PREDSUMU.DBS reads a 64-bit packed source from the even-odd register pair
specified by `rs1_p` and produces a 32-bit scalar result in `rd`. If
`rs1_p = 0`, the source pair is treated as zero.
...
PREDSUMU.DHS reads a 64-bit packed source from the even-odd register pair
specified by `rs1_p` and produces a 32-bit scalar result in `rd`. If
`rs1_p = 0`, the source pair is treated as zero.
...
PMIN.W is encoded in the OP-32 major opcode.
...
of `rs1` and `rs2` and writes the packed word results to `rd`.
...
PMINU.W is encoded in the OP-32 major opcode.
...
PMAX.W is encoded in the OP-32 major opcode.
...
PMAXU.W is encoded in the OP-32 major opcode.
...
msgtz _rd_, _rs2_ &#8594; mslt _rd_, x0, _rs2_
...
packed word elements with a 5-bit unsigned immediate.
...
immediate format with word elements.
...
already within range are passed through unchanged.
...
immediate format with packed halfword elements.
...
out are discarded and vacated bit positions are filled with zeros.
...
for i = 0 .. (XLEN/8 - 1):
    b = s1[(8*i+7):(8*i)]
    d[(8*i+7):(8*i)] = shift_bits_right_arith(b, shamt)
X[rd] = d
----
...
for i = 0 .. (XLEN/16 - 1):
    h = s1[(16*i+15):(16*i)]
    d[(16*i+15):(16*i)] = shift_bits_right_arith(h, shamt)
X[rd] = d
----
...
for i = 0 .. (XLEN/32 - 1):
    w = s1[(32*i+31):(32*i)]
    d[(32*i+31):(32*i)] = shift_bits_right_arith(w, shamt)
X[rd] = d
----
...
for i = 0 .. 7:
    a = s1[(8*i+7):(8*i)]
    d[(8*i+7):(8*i)] = shift_bits_right_arith(a, shamt)
...
for i = 0 .. 3:
    a = s1[(16*i+15):(16*i)]
    d[(16*i+15):(16*i)] = shift_bits_right_arith(a, shamt)
...
for i = 0 .. 1:
    a = s1[(32*i+31):(32*i)]
    d[(32*i+31):(32*i)] = shift_bits_right_arith(a, shamt)
...
w-uimm format with packed byte elements.
...
w-uimm format with packed halfword elements.
...
w-uimm format with word elements.
...
SHA is encoded in the OP-32 major opcode.
...
if (sshamt < 0):
    ...
    if (sshamt <= -64):
        ...
        X[rd] = shift_bits_right_arith(xs1, (0 - shamt)[5:0])[63:0]
else:
    if (sshamt >= 64):
        X[rd] = 0
    else:
...
SHL is encoded in the OP-32 major opcode.
...
for i = 0 .. (XLEN/16 - 1):
    ...
    if sshamt < 0:
        ...
        if sshamt <= -16:
            ...
            r = shift_bits_right_arith(xh, (0 - shamt)[3:0])[15:0]
    else:
...
for i = 0 .. (XLEN/32 - 1):
    ...
    if sshamt < 0:
        ...
        if sshamt <= -32:
            ...
            r = shift_bits_right_arith(xw, (0 - shamt)[4:0])[31:0]
    else:
...
SSHA is encoded in the OP-IMM-32 major opcode.
...
if sshamt < 0:
    if sshamt <= -32:
        ...
        X[rd] = shift_bits_right_arith(xs1, (0 - shamt)[4:0])[31:0]
else:
...
for i = 0 .. 3:
    ...
    if sshamt < 0:
        // Right shift
        d[(16*i+15):(16*i)] = shift_bits_right_arith(to_bits(16, a), -sshamt)
    else:
...
for i = 0 .. 1:
    ...
    if sshamt < 0:
        // Right shift
        d[(32*i+31):(32*i)] = shift_bits_right_arith(to_bits(32, a), -sshamt)
    else:
        // Left shift with saturation
        result = a << sshamt
        if result < -2^31:
...
for i = 0 .. (XLEN/16 - 1):
    ...
    if sshamt < 0:
        ...
        y  = (sshamt <= -16) ? x[32:16]
                             : shift_bits_right_arith(x, (0 - shamt)[3:0])[16:0]
        r  = (y + 1)[16:1]
    else:
        // shift left with signed saturation to 16-bit range
...
for i = 0 .. (XLEN/32 - 1):
    ...
    if sshamt < 0:
        ...
        y  = (sshamt <= -32) ? x[64:32]
                             : shift_bits_right_arith(x, (0 - shamt)[4:0])[32:0]
        r  = (y + 1)[32:1]
    else:
        // shift left with signed saturation to 32-bit range
...
SSHAR is encoded in the OP-IMM-32 major opcode.
...
for i = 0 .. 3:
    ...
    if sshamt < 0:
        ...
        xa = to_bits(16, a) @ 0b0
        shifted = shift_bits_right_arith(xa, -sshamt)
        d[(16*i+15):(16*i)] = (shifted + 1)[16:1]
    else:
        // Left shift with saturation
        result = a << sshamt
        if result < -2^15:
...
SSHL is encoded in the OP-IMM-32 major opcode.
...
result is saturated to the nearest boundary and the `vxsat` flag is set. The
packed word results are written to `rd`.
...
SSHLR is encoded in the OP-IMM-32 major opcode.
...
SHAR is encoded in the OP-32 major opcode.
...
if (sshamt < 0):
    ...
    if (sshamt <= -64):
        ...
        shx = shift_bits_right_arith(xs1, (0 - shamt)[5:0])[64:0]
    X[rd] = (shx + 1)[64:1]
else:
    if (sshamt >= 64):
        X[rd] = 0
...
SHLR is encoded in the OP-32 major opcode.
...
for i = 0 .. (XLEN/16 - 1):
    ...
    shx = shift_bits_right_arith(h @ 0b0, shamt)
...
for i = 0 .. (XLEN/32 - 1):
    w = s1[(32*i+31):(32*i)]
    // Append rounding bit, shift, then extract rounded result
    shx = shift_bits_right_arith(w @ 0b0, shamt)
...
if shamt == 0:
    ...
    // Append rounding bit, shift, then extract rounded result
    shx = shift_bits_right_arith(a @ 0b0, shamt)
    d = (shx + 1)[XLEN:1]
...
for i = 0 .. 3:
    ...
    if shamt == 0:
        ...
        round = a[shamt-1]
        d[(16*i+15):(16*i)] = (shift_bits_right_arith(a, shamt) + round)[15:0]
...
for i = 0 .. 1:
    ...
    if shamt == 0:
        ...
        round = w[shamt-1]
        d[(32*i+31):(32*i)] = shift_bits_right_arith(w, shamt) + round
...
PPAIRE.B is encoded in the OP-32 major opcode with packed byte elements.
...
PPAIRE.B (Pack Pair Even Bytes) interleaves packed 8-bit byte element pairs.
Within each 16-bit pair, the even (lower) byte comes from the even (lower) byte
of `rs1`, while the odd (upper) byte comes from the even (lower) byte of `rs2`.
...
for i = 0 .. (XLEN/16 - 1):
    a_even = s1[(16*i+7):(16*i)]
    b_even = s2[(16*i+7):(16*i)]
    d[(16*i+7):(16*i)] = a_even
    d[(16*i+15):(16*i+8)] = b_even
...
PPAIRE.H (Pack Pair Even Halfwords) interleaves packed 16-bit halfword element
pairs. Within each 32-bit pair, the even (lower) halfword comes from the even
(lower) halfword of `rs1`, while the odd (upper) halfword comes from the even
(lower) halfword of `rs2`. On RV32, this mnemonic is an alias for PACK.
...
for i = 0 .. (XLEN/32 - 1):
    a_even = s1[(32*i+15):(32*i)]
    b_even = s2[(32*i+15):(32*i)]
    d[(32*i+15):(32*i)] = a_even
    d[(32*i+31):(32*i+16)] = b_even
...
register-pair format with packed byte elements.
...
PPAIRE.DB performs packed byte even-element interleave on double-wide
...
register-pair format with packed halfword elements.
...
PPAIRE.DH performs packed halfword even-element interleave on double-wide
...
PPAIREO.B is encoded in the OP-32 major opcode with packed byte elements.
...
PPAIREO.B (Pack Pair Even-Odd Bytes) interleaves packed 8-bit byte element
pairs. Within each 16-bit pair, the even (lower) byte comes from the even
(lower) byte of `rs1`, while the odd (upper) byte comes from the odd (upper)
byte of `rs2`.
...
for i = 0 .. (XLEN/16 - 1):
    a_even = s1[(16*i+7):(16*i)]
    b_odd = s2[(16*i+15):(16*i+8)]
    d[(16*i+7):(16*i)] = a_even
    d[(16*i+15):(16*i+8)] = b_odd
...
PPAIREO.H (Pack Pair Even-Odd Halfwords) interleaves packed 16-bit halfword
element pairs. Within each 16-bit pair, the even (lower) halfword comes from the
even (lower) halfword of `rs1`, while the odd (upper) halfword comes from the
odd (upper) halfword of `rs2`.
...
PPAIREO.W (Pack Pair Even-Odd Words) interleaves a packed 32-bit word element
pair. Within the 32-bit pair, the even (lower) word comes from the even (lower)
word of `rs1`, while the odd (upper) word comes from the odd (upper) word of
`rs2`.
...
PPAIREO.DB performs packed byte even-odd element interleave on double-wide
...
PPAIREO.DH performs packed halfword even-odd element interleave on double-wide
...
PPAIROE.B is encoded in the OP-32 major opcode with packed byte elements.
...
PPAIROE.B (Pack Pair Odd-Even Bytes) interleaves packed 8-bit byte element
pairs. Within each 16-bit pair, the even (lower) byte comes from the odd
(upper) byte of `rs1`, while the odd (upper) byte comes from the even (lower)
byte of `rs2`.
...
PPAIROE.H (Pack Pair Odd-Even Halfwords) interleaves packed 16-bit halfword
element pairs. Within each 16-bit pair, the even (lower) halfword comes from the
odd (upper) halfword of `rs1`, while the odd (upper) halfword comes from the
even (lower) halfword of `rs2`.
...
PPAIROE.W (Pack Pair Odd-Even Words) interleaves a packed 32-bit word element
pair. Within the 32-bit pair, the even (lower) word comes from the odd (upper)
word of `rs1`, while the odd (upper) word comes from the even (lower) word of
`rs2`.
...
PPAIROE.DB performs packed byte odd-even element interleave on double-wide
...
PPAIROE.DH performs packed halfword odd-even element interleave on double-wide
...
PPAIRO.B is encoded in the OP-32 major opcode with packed byte elements.
...
PPAIRO.B (Pack Pair Odd Bytes) interleaves packed 8-bit byte element pairs.
Within each 16-bit pair, the even (lower) byte comes from the odd (upper) byte
of `rs1`, while the odd (upper) byte comes from the odd (upper) byte of `rs2`.
...
PPAIRO.H (Pack Pair Odd-Even Halfwords) interleaves packed 16-bit halfword
element pairs. Within each 16-bit pair, the even (lower) halfword comes from the
odd (upper) halfword of `rs1`, while the odd (upper) halfword comes from the
odd (upper) halfword of `rs2`.
...
PPAIRO.W (Pack Pair Odd Words) interleaves a packed 32-bit word element pair.
Within the 32-bit pair, the even (lower) word comes from the odd (upper)
word of `rs1`, while the odd (upper) word comes from the odd (upper) word of
`rs2`.
...
PPAIRO.DB performs packed byte odd-element interleave on double-wide
...
PPAIRO.DH performs packed halfword odd-element interleave on double-wide
...
ZIP8P is encoded in the OP-32 major opcode.
...
ZIP8HP is encoded in the OP-32 major opcode.
...
ZIP16P is encoded in the OP-32 major opcode.
...
ZIP16HP is encoded in the OP-32 major opcode.
...
UNZIP8P is encoded in the OP-32 major opcode.
...
UNZIP8HP is encoded in the OP-32 major opcode.
...
UNZIP16P is encoded in the OP-32 major opcode.
...
UNZIP16HP is encoded in the OP-32 major opcode.
...
REV16 is a scalar instruction encoded in the OP-IMM major opcode.
...
ABSW is encoded in the OP-IMM-32 major opcode.
...
CLSW is encoded in the OP-IMM-32 major opcode.
...
s2 = (rs2_p == 0) ? 0 : (X[2*rs2_p+1] @ X[2*rs2_p])
...
PWADD.B writes a 64-bit result to an even-odd register pair specified by `rd_p`.
...
PWADD.H writes results to an even-odd register pair specified by `rd_p`.
...
WADD writes a 64-bit result to register pair `rd_p`.
...
PWADDU.B writes a 64-bit result to an even-odd register pair specified by
`rd_p`.
...
PWSUB.B writes a 64-bit result to an even-odd register pair specified by `rd_p`.
...
PWSUBU.B writes a 64-bit result to an even-odd register pair specified by
`rd_p`.
...
PWADDA.B accumulates into the destination register pair.
...
PWADDA.H accumulates into the destination register pair.
...
WADDA accumulates into the destination register pair.
...
PWADDAU.B accumulates into the destination register pair.
...
PWADDAU.H accumulates into the destination register pair.
...
WADDAU accumulates into the destination register pair.
...
PWSUBA.B accumulates into the destination register pair.
...
PWSUBA.H accumulates into the destination register pair.
...
WSUBA accumulates into the destination register pair.
...
PWSUBAU.B accumulates into the destination register pair.
...
PWSUBAU.H accumulates into the destination register pair.
...
WSUBAU accumulates into the destination register pair.
...
The shift amount is encoded in the `uimm4` field. The destination is a register
pair `rd_p`.
...
The shift amount is encoded in `uimm5`.
...
The shift amount is encoded in `uimm6`.
...
The shift amount is taken from `rs2`.
...
The shift amount is encoded in `uimm4`.
...
WZIP8P writes its 64-bit result to the even-odd register pair specified by `rd_p`
(no write occurs if `rd_p = 0`).
It is encoded in the RV32 register-pair OP-IMM-32 format with `f=111` and `w=00`.
...
WZIP16P writes its 64-bit result to the even-odd register pair specified by
`rd_p` (no write occurs if `rd_p = 0`).
It is encoded in the RV32 register-pair OP-IMM-32 format with `f=111` and `w=01`.
...
for i = 0 .. 3:
    h = s1[(16*i+15):(16*i)]
    d[(8*i+7):(8*i)] = shift_bits_right_arith(h, shamt)[7:0]
...
d[15:0]  = shift_bits_right_arith(s1[31:0], shamt)[15:0]
d[31:16] = shift_bits_right_arith(s1[63:32], shamt)[15:0]
...
s1    = (rs1_p == 0) ? 0 : (X[2*rs1_p+1] @ X[2*rs1_p])
shamt = uimm6[5:0]
X[rd] = shift_bits_right_arith(s1, shamt)[31:0]
----
...
for i = 0 .. 3:
    h  = s1[(16*i+15):(16*i)]
    // Append a rounding bit, shift arithmetically, then add 1 and drop the LSB
    shx = shift_bits_right_arith(h @ 0b0, shamt)[8:0]
...
for i = 0 .. 1:
    w   = s1[(32*i+31):(32*i)]
    shx = shift_bits_right_arith(w @ 0b0, shamt)[16:0]
    d[(16*i+15):(16*i)] = (shx + 1)[16:1]
...
shx = shift_bits_right_arith(s1 @ 0b0, shamt)[32:0]
X[rd] = (shx + 1)[32:1]
----
...
for i = 0 .. 3:
    h   = s1[(16*i+15):(16*i)]                  // 16-bit lane
    shx = shift_bits_right_arith(h, shamt)  // 16-bit arithmetic result
...
amount and packed halfword narrowing.
...
for i = 0 .. 1:
    ...
    shx  = shift_bits_right_arith(s1_w, shamt)  // 32-bit arithmetic result
    if (shx <_s 0xFFFF8000):
        d[(16*i+15):(16*i)] = 0x8000; vxsat = 1
    else if (shx >_s 0x00007FFF):
        d[(16*i+15):(16*i)] = 0x7FFF; vxsat = 1
...
shx = shift_bits_right_arith(s1, shamt)  // 64-bit arithmetic result
...
for i = 0 .. 3:
    ...
    shx  = shift_bits_right_arith(s1_h, shamt)  // 16-bit arithmetic result
    if (shx <_s 0xFF80):
...
for i = 0 .. 3:
    ...
    // Append a rounding bit, shift, then add 1 and drop the LSB
    shx = shift_bits_right_arith(h @ 0b0, shamt)      // keep 17 bits
...
amount, rounding, and packed halfword narrowing.
...
for i = 0 .. 1:
    ...
    shx       = shift_bits_right_arith(s1_w @ 0b0, shamt)   // 33-bit result
    round_shx = (shx + 1)[32:1]                           // round to nearest, ties up
    if (round_shx <_s 0xFFFF8000):
        d[(16*i+15):(16*i)] = 0x8000; vxsat = 1
    else if (round_shx >_s 0x00007FFF):
        d[(16*i+15):(16*i)] = 0x7FFF; vxsat = 1
    else:
...
amount, rounding, and XLEN-wide narrowing.
...
// Shift right with rounding: append guard bit, shift, then add 1 and drop LSB
shx       = shift_bits_right_arith(s1 @ 0b0, shamt)   // 65-bit result
round_shx = (shx + 1)[64:1]                          // round to nearest, ties up

if (round_shx <_s 0xFFFFFFFF80000000):
    X[rd] = 0x80000000; vxsat = 1
...
rounding, and XLEN-wide narrowing.
...
amount and unsigned XLEN-wide narrowing.
...
amount, rounding, and unsigned XLEN-wide narrowing.
...
if (round_shx >_u 0x00000000FFFFFFFF):
    X[rd] = 0xFFFFFFFF; vxsat = 1
else:
    X[rd] = round_shx[31:0]
----
...
rounding, and unsigned XLEN-wide narrowing.
...
PMULH.W is encoded in the OP-32 major opcode with packed word elements.
...
of each product to the corresponding word element of `rd`.
...
PMULHR.W is encoded in the OP-32 major opcode with packed word elements.
...
corresponding word element of `rd`.
...
PMULQ.W is encoded in the OP-32 major opcode with packed word elements.
...
result saturates to 0x7FFFFFFF and the `vxsat` overflow flag is set.
...
PMULQR.W is encoded in the OP-32 major opcode with packed word elements.
...
0x7FFFFFFF and the `vxsat` overflow flag is set.
...
PMUL.W.H00 is encoded in the OP-32 major opcode with packed word elements.
...
to the corresponding word of `rd`.
...
PMUL.W.H01 is encoded in the OP-32 major opcode with packed word elements.
...
PMUL.W.H11 is encoded in the OP-32 major opcode with packed word elements.
...
written to the corresponding word of `rd`.
...
MUL.W00 is encoded in the OP-32 major opcode as a cross-word multiply.
...
MUL.W01 is encoded in the OP-32 major opcode as a cross-word multiply.
...
MUL.W11 is encoded in the OP-32 major opcode as a cross-word multiply.
...
PM2SUB.W is encoded in the OP-32 major opcode with word elements.
...
PM2SUB.W multiplies two pairs of signed 32-bit word elements from
...
PM2SUB.WX multiplies two pairs of signed 32-bit word elements from
...
PM2ADDA.W is encoded in the OP-32 major opcode with word elements.
...
PM2ADDA.W multiplies two pairs of signed 32-bit word elements from
...
PM2ADDAU.W is encoded in the OP-32 major opcode with word elements.
...
PM2ADDAU.W multiplies two pairs of unsigned 32-bit word elements
...
PM4ADDA.H multiplies four pairs of signed 16-bit halfword elements
...
PMULH.W.H0 is encoded in the OP-32 major opcode with packed word elements.
...
each product to the corresponding word of `rd`.
...
PMULH.W.H1 is encoded in the OP-32 major opcode with packed word elements.
...
[47:16] of each product to the corresponding word of `rd`.
...
All instructions that write VXSAT are excluded. Averaging, count sign bits,
maximum, minimum, reductions, and horizontal multiply-add are also excluded.

The RV32 and RV64 columns below indicate the required XLEN for the instructions.
There is no case where an instruction is supported for both XLEN values, but
only part of Zkt for one of the XLen value.
